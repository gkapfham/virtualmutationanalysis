...
visualise_mutation_score_time_constrained <- function(data) {
  p = ggplot2::ggplot(data,ggplot2::aes(x = score_type, y = score)) +
    ggplot2::facet_grid(dbms~schema, labeller = ggplot2::label_parsed) +
    ggplot2::geom_boxplot(outlier.size = 0.75, lwd = 0.25) +
    ggplot2::theme_grey(base_size = 2) +
    ggplot2::theme(title = ggplot2::element_text(size=2)) +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1, size = 2)) +
    ggplot2::theme(axis.text.y = ggplot2::element_text(angle = 45, hjust = 1, size = 2)) +
